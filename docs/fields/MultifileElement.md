...
class MultifileElementForm extends VueFormBuilder
{
    protected function buildForm()
    {
        return Vueform::build()
            ->schema([
                MultifileElement::name('attachments')
                    ->description('Please upload multiple files using the field below.')
            ]);
    }
}
```
...
The following slots <a href="https://vueform.com/reference/multifile-element#slots" target="_blank">Documentation</a> are available for this element:
...
| `before` | - {component} el$ - the element's component | Renders an ElementText component before the file list. |
| `between` | - {component} el$ - the element's component | Renders an ElementText component after the file list and before description. |
